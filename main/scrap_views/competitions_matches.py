    ...
    try:
        ...
        for matchday in matchday_lists:
            try:
                ...
                for match_elem in match_elements:
                    try:
                        ...
                        SeasonMatch.objects.get_or_create(
                            season=season,
                            match=match,
                            defaults={'round': current_round}
                        )

                        if created:
                            stats['created'] += 1
                            logger.info(f"✅ Created match: {home_team_name} vs {away_team_name}")
                        else:
                            stats['updated'] += 1
                            logger.info(f"🔄 Updated match: {home_team_name} vs {away_team_name}")
                ...
                logger.error(f"Error processing matchday: {e}")
                continue
